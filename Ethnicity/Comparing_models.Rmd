...
df_Ayushi <- read_csv("/Users/amirrshams/Library/CloudStorage/OneDrive-UniversityofWaterloo/Thesis/Dataset/Ayushi_Dataset/new_pullreq.csv")
...
set.seed(44)
category <- 'Replaced'
df_nonmerged_factor_rep <- df_nonmerged[df_nonmerged$manual_analysis == category, ]
...
for (i in 1:num_bootstraps) {
  ...
  # Combine the "Unnecessary" pull requests with the bootstrapped merged data
  combined_data_boot <- rbind(df_nonmerged_factor_rep, bootstrap_sample)

  # Refit the model on the combined bootstrapped data
  boot_model_rep <- glmer(formula, data = combined_data_boot, family = binomial,
                      control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE, optCtrl = list(maxeval = 50)))
  bootstrapped_models[[i]] <- boot_model_rep

  # Extract and store the model parameters
  bootstrapped_parameters[[i]] <- coef(boot_model_rep)
}
...
```{r}
for (i in 1:num_bootstraps) {
  model_summary_rep <- summary(bootstrapped_models[[i]])
  #conf_intervals <- confint(bootstrapped_models[[i]], which = "fixed")
...
set.seed(44)
category <- 'Duplicate'
df_nonmerged_factor_dup <- df_nonmerged[df_nonmerged$manual_analysis == category, ]
...
for (i in 1:num_bootstraps) {
  ...
  # Combine the "Duplicate" pull requests with the bootstrapped merged data
  combined_data_boot_dup <- rbind(df_nonmerged_factor_dup, bootstrap_sample)

  # Refit the model on the combined bootstrapped data
  boot_model_rep <- glmer(formula, data = combined_data_boot_dup, family = binomial,
                      control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE, optCtrl = list(maxeval = 50)))
  bootstrapped_models[[i]] <- boot_model_rep
...
```{r}
for (i in 1:num_bootstraps) {
  model_summary_dup <- summary(bootstrapped_models[[i]])
  #conf_intervals <- confint(bootstrapped_models[[i]], which = "fixed")
...
df_nonmerged_factor_mc <- df_nonmerged[df_nonmerged$manual_analysis == category, ]
...
for (i in 1:num_bootstraps) {
  ...
  # Combine the "Duplicate" pull requests with the bootstrapped merged data
  combined_data_boot_mc <- rbind(df_nonmerged_factor_mc, bootstrap_sample)

  # Refit the model on the combined bootstrapped data
  boot_model_mc <- glmer(formula, data = combined_data_boot_mc, family = binomial,
                      control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE, optCtrl = list(maxeval = 50)))
  bootstrapped_models[[i]] <- boot_model_mc

  # Extract and store the model parameters
  bootstrapped_parameters[[i]] <- coef(boot_model_mc)
}
...
```{r}
for (i in 1:num_bootstraps) {
  model_summary_mc <- summary(bootstrapped_models[[i]])
  conf_intervals <- confint(boot_model_mc, method = "Wald")
...
set.seed(43)
category <- 'Stale'
df_nonmerged_factor_sta <- df_nonmerged[df_nonmerged$manual_analysis == category, ]
...
for (i in 1:num_bootstraps) {
  ...
  # Combine the "Stale" pull requests with the bootstrapped merged data
  combined_data_boot_sta <- rbind(df_nonmerged_factor_sta, bootstrap_sample)

  # Refit the model on the combined bootstrapped data
  boot_model_sta <- glmer(formula, data = combined_data_boot_sta, family = binomial,
                      control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE, optCtrl = list(maxeval = 50)))
  bootstrapped_models[[i]] <- boot_model_sta

  # Extract and store the model parameters
  bootstrapped_parameters[[i]] <- coef(boot_model_sta)
}
...
```{r}
for (i in 1:num_bootstraps) {
  model_summary_sta <- summary(bootstrapped_models[[i]])
  conf_intervals <- confint(bootstrapped_models[[i]], method = "Wald")
