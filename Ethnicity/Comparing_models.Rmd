...
## selecting the category, getting the data for that category and Factoring the data
...
# Filter the merged pull requests to keep only those with repo_ids in unique_repo_ids
df_merged_factored <- df_merged_factored[df_merged_factored$repo_id %in% unique_repo_ids, ]



formula <- status ~  comments_counts + commit_counts + code_changes_counts + author_continent  + author_eth + closer_eth  + prs_pri_same_nationality + prs_experience + prs_succ_rate + prs_popularity + prs_watched_repo + prs_tenure_mnth + prs_main_team_member +
repo_pr_tenure_mnth + repo_pr_popularity + repo_pr_team_size + perc_external_contribs + intra_branch + (1 | repo_id) + (1 | author_id)

# Define a function to fit the glmer model
fit_glmer_model <- function(data) {
  # Combine the "Unnecessary" pull requests with the merged pull requests
  combined_data <- rbind(df_nonmerged_factor_unn, data)
  
  # Fit your glmer model on the combined data
...
num_bootstraps <- 2  # You can adjust this number as needed

# Create a vector to store bootstrapped model results
boot_models <- vector("list", length = num_bootstraps)

# Perform bootstrapping
for (i in 1:num_bootstraps) {
  # Generate bootstrap indices by sampling with replacement from merged pull requests
  boot_indices <- sample(1:nrow(df_merged_factored), replace = TRUE)
  
  # Extract the bootstrapped sample of merged pull requests
  bootstrap_sample <- df_merged_factored[boot_indices, ]
  
  # Check if the repo_id of each sample is in the unique_repo_ids from "Unnecessary" pull requests
  # If not, resample until a matching repo_id is found
  #while (!(bootstrap_sample$repo_id %in% unique_repo_ids)) {
   # boot_indices <- sample(1:nrow(df_merged_factored), replace = TRUE)
    #bootstrap_sample <- df_merged_factored[boot_indices, ]
  colnames(bootstrap_sample) <- colnames(df_nonmerged_factor_unn)

  
  # Fit the glmer model on the bootstrapped merged data
  boot_models[[i]] <- fit_glmer_model(bootstrap_sample)
}

# Display the bootstrapped models
print(boot_models)
...
for (i in 1:num_bootstraps) {
  ...
  print(combined_data_boot[2, ])
  print(combined_data_boot[7000, ])
  # Refit the model on the combined bootstrapped data
  boot_model <- glmer(formula, data = combined_data_boot, family = binomial,
                      control = glmerControl(optimizer = "nloptwrap", calc.derivs = FALSE, optCtrl = list(maxeval = 50)))
  bootstrapped_models[[i]] <- boot_model

  # Extract and store the model parameters
  bootstrapped_parameters[[i]] <- coef(boot_model)
}
...
```{r}
for (i in 1:num_bootstraps) {
  model_summary <- summary(bootstrapped_models[[i]])
